...
def _profile_section(styles, profile: dict) -> list:
    ...
    # Keys match intake_agent.py INTAKE_QUESTIONS exactly
    currently_in_nz = profile.get("currently_in_nz")
    if isinstance(currently_in_nz, bool):
        location_val = "Currently in New Zealand" if currently_in_nz else "Outside New Zealand"
    else:
        location_val = str(currently_in_nz) if currently_in_nz else "—"
 
    job_offer = profile.get("job_offer")
    if isinstance(job_offer, bool):
        job_offer_val = "Yes" if job_offer else "No"
    else:
        job_offer_val = str(job_offer) if job_offer else "—"
    ...
    fields = [
        ...
        ("Occupation / Job Title", profile.get("occupation", "—")),
        ("NZ Job Offer", job_offer_val),
        ...
        ("NZ Work Experience (yrs)", profile.get("nz_work_experience_years", "—")),
        ("English Proficiency", profile.get("english_level", "—")),
        ("ANZSCO Code", profile.get("anzsco_code", "—")),
        ("Family Members", profile.get("family", "—")),
    ]
...
def _assessment_section(styles, assessment: dict) -> list:
    ...
    # classification_agent nests all parsed fields under assessment["parsed"]
    parsed = assessment.get("parsed", assessment)  # fallback to flat dict if called directly
 
    total = parsed.get("total_points", "—")
    pillar_points = parsed.get("pillar_points", "—")
    pillar = parsed.get("pillar", "—")
    nz_exp_points = parsed.get("nz_experience_points", "0")
    status = parsed.get("status", "UNKNOWN")
    strengths = parsed.get("strengths", [])
    gaps = parsed.get("gaps", [])
    actions = parsed.get("recommended_actions", [])
    risk_flags = [r for r in parsed.get("risk_flags", []) if r.strip().lower() not in ("none", "none.")]
    breakdown = {}  # SMC uses pillar system, not category breakdown
    reasoning = ""
    ...
    banner_data = [[
        Paragraph(f"<b>{total} / 6 points</b>", styles["points_big"]),
    ...
    # SMC pillar breakdown table
    story.append(Paragraph("<b>Points Breakdown</b>", styles["body"]))
    breakdown_rows = [
        ["Component", "Points", "Max"],
        [f"Main Pillar ({pillar.title() if pillar != '—' else 'Not set'})", str(pillar_points), "6"],
        ["NZ Work Experience Top-up", str(nz_exp_points), "3"],
        ["TOTAL", str(total), "6 (min required)"],
    ]
    t = Table(breakdown_rows, colWidths=[100*mm, 30*mm, 35*mm])
    t.setStyle(TableStyle([
